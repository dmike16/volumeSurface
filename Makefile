...
volume-init := $(init-y)
volume-main := $(core-y) $(libs-y) $(drivers-y)
volume-all  := $(volume-init) $(volume-main)
export KBUILD_PVSCHEMA_OBJS := $(volume-all)

volume.out: $(volume-init) $(volume-main)
	$(CC) -o arch/$@ $^ $(SYSTEM-LIBRARIES)


$(sort $(volume-init) $(volume-main)): $(volume-dirs) ;
...
clean-dirs	:= $(addprefix _clean_, . $(volume-alldirs))
